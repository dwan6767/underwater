//**************************************************************
// * Unified Master/Slave — FINAL (common threshold = 400)
...
/* ---------------- Pin mapping ---------------- */
#ifdef ROLE_MASTER
  const uint8_t PIN_RX_A = A0;            // ESP8266 A0
  const uint8_t PIN_TX_DIGITAL = D2;      // ESP8266 D2 (GPIO4)
#else
  const uint8_t PIN_RX_A = A0;            // Nano A0
  const uint8_t PIN_TX_DIGITAL = 8;       // Nano D8
#endif

/* ---------------- Threshold ---------------- */
const int ADC_THRESHOLD = 400; // both master & slave use >400 as active

/* ---------------- Protocol & timing ---------------- */
const float TONE_SLAVE_HZ  = 2.0f;    // slave-control (request/ack)
const float TONE_MASTER_HZ = 4.0f;    // master-control

const unsigned long CONTROL_TONE_DURATION_MS = 3000UL;
const float CONTROL_TONE_TOLERANCE_HZ = 0.4f;
const int CONTROL_TONE_CONFIRM = 4;

#define LETTER_ON_MS 1200UL
#define GAP_MS 250UL
#define SETTLE_MS 60UL
...
#ifdef ROLE_SLAVE
const char SLAVE_DEFAULT_MSG[] = "CHECKMATE";
